...
export function AIInsightsDashboard() {
  ...
  const [selectedTimeHorizon, setSelectedTimeHorizon] = useState<"1h" | "4h" | "24h" | "7d">("24h");
  ...
  const loadAIData = useCallback(async () => {
    try {
      ...
      const [predictions, opportunities, insights] = await Promise.all([
        aiForecastingEngine.predictPrice(selectedEnergyType, selectedTimeHorizon),
      ...
      const forecasts = await Promise.all(
        energyTypes.map(type => aiForecastingEngine.forecastSurplus(type, selectedTimeHorizon))
      );
  ...
  const getOpportunityIcon = (type: string) => {
    switch (type) {
      ...
      case "high_availability": return Zap;
      case "low_competition": return Target;
  ...
  const formatPredictionData = () => {
    return pricePredictions.map((pred, index) => ({
      time: pred.timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
    ...
    return (
      <Card className="bg-white/60 backdrop-blur-sm">
        <CardContent className="p-8 text-center">
          ...
          <p className="text-gray-500">Sign in to access advanced forecasting and market insights.</p>
        </CardContent>
      </Card>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Advanced forecasting and predictive analytics for energy markets</p>
        </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <Badge className={`mt-1 ${marketInsights ? getMarketTrendColor(marketInsights.marketTrend) : 'bg-gray-100'}`}>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-blue-600">{marketOpportunities.length}</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-orange-600">{marketInsights?.riskLevel || 0}%</p>
              </div>
                ...
                <CardHeader>
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <Select value={selectedTimeHorizon} onValueChange={(value: any) => setSelectedTimeHorizon(value)}>
                  ...
                  {pricePredictions.length > 0 && (
                    <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                      <div className="text-center">
                        ...
                        <p className="font-bold text-lg">${pricePredictions[0]?.predictedPrice.toFixed(3)}</p>
                      </div>
                      <div className="text-center">
                        <p className="text-gray-500">Peak Price</p>
                        <p className="font-bold text-lg text-red-600">
                          ${Math.max(...pricePredictions.map(p => p.predictedPrice)).toFixed(3)}
                        </p>
                      </div>
                      <div className="text-center">
                        <p className="text-gray-500">Low Price</p>
                        <p className="font-bold text-lg text-green-600">
                          ${Math.min(...pricePredictions.map(p => p.predictedPrice)).toFixed(3)}
                        </p>
                      </div>
                      <div className="text-center">
                        <p className="text-gray-500">Avg. Confidence</p>
                        <p className="font-bold text-lg text-purple-600">
                          {Math.round(pricePredictions.reduce((sum, p) => sum + p.confidence, 0) / pricePredictions.length)}%
                        </p>
                      </div>
                    </div>
                  )}
              ...
              <Card className="bg-white/60 backdrop-blur-sm">
                ...
                <CardContent className="space-y-4">
                  {marketInsights && (
                    <>
                      <div>
                        <p className="text-sm font-medium text-gray-500 mb-1">Liquidity Score</p>
                        <div className="flex items-center space-x-2">
                          <Progress value={marketInsights.liquidityScore} className="flex-1" />
                          <span className="text-sm font-bold">{marketInsights.liquidityScore}%</span>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="flex items-center space-x-2">
                          <Progress value={marketInsights.competitionLevel} className="flex-1" />
                          <span className="text-sm font-bold">{marketInsights.competitionLevel}%</span>
                        </div>
                      </div>

                      <div>
                        <p className="text-sm font-medium text-gray-500 mb-1">Volatility</p>
                        <Badge className={
                          marketInsights.volatility === 'high' ? 'bg-red-100 text-red-800' :
                          marketInsights.volatility === 'medium' ? 'bg-orange-100 text-orange-800' :
                      ...
                      <div>
                        <p className="text-sm font-medium text-gray-500 mb-1">Recommended Strategy</p>
                        <Badge className={
                          marketInsights.recommendedStrategy === 'aggressive' ? 'bg-red-100 text-red-800' :
                          marketInsights.recommendedStrategy === 'conservative' ? 'bg-blue-100 text-blue-800' :
                          'bg-green-100 text-green-800'
                        }>
                      ...
                      <div>
                        <p className="text-sm font-medium text-gray-500 mb-2">Key Factors</p>
                        <div className="space-y-1">
                          {marketInsights.keyFactors.map((factor, index) => (
        ...
        <TabsContent value="opportunities" className="mt-6">
          <Card className="bg-white/60 backdrop-blur-sm">
            <CardHeader>
              ...
              <CardDescription>
                Real-time analysis of trading opportunities based on market conditions
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {marketOpportunities.length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-500">AI is monitoring the market for new opportunities.</p>
                  </div>
                ) : (
                  marketOpportunities.map((opportunity) => {
                    const OpportunityIcon = getOpportunityIcon(opportunity.opportunityType);
                    return (
                      <div
                        key={opportunity.id}
                      ...
                      >
                        <div className="flex items-start justify-between mb-3">
                          <div className="flex items-center space-x-3">
                            ...
                            <div>
                              <h3 className="font-semibold">{opportunity.energyType} - {opportunity.location}</h3>
                              <p className="text-sm opacity-80">{opportunity.description}</p>
                            </div>
                          </div>
                          <div className="flex items-center space-x-2">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-3 text-sm">
                          <div>
                            ...
                            <p className="font-bold">${opportunity.potentialSavings.toFixed(0)}</p>
                          </div>
                          <div>
                            <p className="opacity-70">Time to Act</p>
                            <p className="font-bold">{opportunity.timeToAct}m</p>
                          </div>
                          <div>
                            <p className="opacity-70">Opportunity Type</p>
                            <p className="font-bold capitalize">{opportunity.opportunityType.replace('_', ' ')}</p>
                          </div>
                          <div className="flex items-center">
                            <Button
                              ...
                              onClick={() => {
                                ...
                                if (opportunity.recommendedAction.toLowerCase().includes('bid')) {
                                  const biddingTab = document.querySelector('[value="bidding"]') as HTMLElement;
                                  if (biddingTab) {
                                    biddingTab.click();
                                  }
                                } else if (opportunity.recommendedAction.toLowerCase().includes('watch')) {
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {surplusForecasts.map((forecast) => (
                  <div key={forecast.energyType} className="border rounded-lg p-4 bg-gray-50">
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="font-semibold text-lg">{forecast.energyType}</h3>
                    ...
                    <div className="space-y-3">
                      <div>
                        <p className="text-sm text-gray-500">Expected Surplus</p>
                        <p className="text-xl font-bold text-green-600">{forecast.expectedSurplus} MWh</p>
                      </div>

                      <div>
                        <p className="text-sm text-gray-500">Curtailment Risk</p>
                        <div className="flex items-center space-x-2">
                          <Progress value={forecast.curtailmentRisk} className="flex-1" />
                          <span className="text-sm font-bold">{forecast.curtailmentRisk}%</span>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1">
                          {forecast.peakTimes.map((time, index) => (
        ...
        <TabsContent value="models" className="mt-6">
          <Card className="bg-white/60 backdrop-blur-sm">
            ...
            <CardContent>
              <div className="space-y-4">
                {models.map((model) => (
                  <div key={model.name} className="border rounded-lg p-4 bg-gray-50">
                    <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="mt-3">
                      <p className="text-xs text-gray-500 mb-1">Training Features:</p>
                      <div className="flex flex-wrap gap-1">
              ...
              <div className="mt-6 p-4 bg-blue-50 rounded-lg">
                <h4 className="font-semibold text-blue-800 mb-2">Model Training Information</h4>
                <p className="text-sm text-blue-700">
                  AI models are continuously trained on market data to improve prediction accuracy.
                  Training incorporates weather patterns, demand cycles, generation capacity, and historical price movements.
                </p>
              </div>
            </CardContent>
          </Card>
